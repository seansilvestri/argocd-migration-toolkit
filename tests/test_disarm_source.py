#!/usr/bin/env python3
"""
Unit tests for disarm-source.sh script using Python for reliable error handling.
"""
...
def test_disarm_source():
    """Test the disarm-source.sh script."""
...
""")
        ...
        try:
            ...
            script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../scripts/migration/disarm-source.sh")
            ...
            if result.returncode != 0:
                print(f"❌ disarm-source.sh failed: {result.stderr}")
                return False
            
            print("✅ disarm-source.sh executed successfully in dry-run mode")
            
            # Test with missing source directory
            # Remove all files first, then the directory
