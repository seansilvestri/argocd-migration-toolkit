...
source scripts/migration/migration-env.sh "${SCRIPT_DIR}/test-configs/appset.env"
...
# Wait for source workload-cluster.apps to be created and synced by root app
echo "  Waiting for workload-cluster.apps to be created and synced..."
for i in {1..30}; do
    if kubectl get application workload-cluster.apps -n argocd --context kind-source-argocd > /dev/null 2>&1; then
        # Also wait for it to sync the ApplicationSet
        if kubectl get applicationset workload-cluster.guestbook-appset -n argocd --context kind-source-argocd > /dev/null 2>&1; then
            echo "  ✅ workload-cluster.apps created and ApplicationSet synced"
            break
        fi
    fi
    sleep 2
...
# Wait for ApplicationSet to generate child applications
echo "  Waiting for ApplicationSet to generate child applications..."
for i in {1..30}; do
    CHILD_COUNT=$(kubectl get applications -n argocd --context kind-source-argocd -o json | jq '[.items[] | select(.metadata.name | startswith("workload-cluster.guestbook-"))] | length')
    if [ "$CHILD_COUNT" -ge 2 ]; then
        echo "  ✅ ApplicationSet generated $CHILD_COUNT child applications"
        break
    fi
    sleep 2
done
...
# Step 5.5: Verify disarm (check ApplicationSet, parent keeps finalizers until Commit B)
echo "🔍 Verifying disarm..."
kubectl config use-context kind-source-argocd
if kubectl get applicationset workload-cluster.guestbook-appset -n argocd > /dev/null 2>&1; then
    echo "  ✅ ApplicationSet still exists (will be deleted in cleanup)"
fi
...
apply_commit_b_changes "${WORK_DIR}"
...
print_summary "ApplicationSet Migration" \
    "Commit A: Disabled auto-sync and created target manifests" \
    "Disarm: Removed finalizers from source ApplicationSet and children" \
    "Target synced and healthy" \
    "Cleanup: Deleted source ApplicationSet and children" \
